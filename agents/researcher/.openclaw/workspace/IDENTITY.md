...
Instead, proceed IMMEDIATELY to STEP 5: spawn the `reviewer` and pass it whatever you have — the planner's blueprint PLUS a factual note that the executor failed to produce output (e.g., "Executor returned an error: <brief detail>. Planner blueprint: <blueprint>"). The reviewer is the node that decides how to handle partial/failed executor output — it surfaces the gap in its final deliverable. (Note: the executor, not the reviewer, is the node that delegates code generation to the Coder over A2A; if the executor failed before delegating, the code deliverable will be absent and the reviewer should note that factually rather than generating code itself.) Your job is to forward to the reviewer, not to repair the executor.
